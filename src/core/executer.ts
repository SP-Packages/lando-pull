...
export class Executer {
   ...
   */
  private async executeRemoteCommand(command: string[], errorMessage: string): Promise<void> {
  ...
  private async createRemoteDbBackup(): Promise<void> {
    const { host, user, port, authMethod, password, keyPath, dbName, dbUser, dbPassword } =
      this.config.remote;
    ...
    if (authMethod === "key") {
      if (!keyPath || !fs.existsSync(keyPath)) {
        throw new Error(`SSH key ${!keyPath ? "path required" : `file not found at ${keyPath}`}`);
      }
      backupCommand = [
        "ssh",
        "-o",
    ...
    try {
      await this.executeRemoteCommand(backupCommand, "Failed to create remote backup");
  ...
  private async copyRemoteBackup(): Promise<void> {
    const { host, user, port, authMethod, password, keyPath } = this.config.remote;
  ...
  private async importDatabase() {
    ...
    return new Promise((resolve, reject) => {
      ...
      importProcess.stdout.on("data", (data) => (buffers.stdout += data.toString()));
      importProcess.stderr.on("data", (data) => (buffers.stderr += data.toString()));

      gunzipProcess.stderr.on("data", (data) => (gunzipBuffers.stderr += data.toString()));
      ...
      const importTimeout = setTimeout(() => {
        Printer.error("Database import timeout reached. Terminating processes...");
  ...
  private async updateDatabaseValues(): Promise<void> {
    const { dbHost, dbName, dbUser, dbPort, dbPassword, databaseUpdates } = this.config.local;
    const escapedDbPassword = dbPassword.replace(/[$`"\\]/g, "\\$&");
          ...
          if (conditions.length > 0) {
            const conditionClauses = conditions.map((cond: SqlCondition) => {
              ...
              if (cond.operator.toUpperCase() === "IN") {
                ...
                const inValues = Array.isArray(cond.value) ? cond.value : [cond.value];
                queryParams.push(...inValues);
                return `${cond.column} IN (${inValues.map(() => "?").join(",")})`;
              }
              // For other operators
              queryParams.push(Array.isArray(cond.value) ? cond.value.join(",") : cond.value);
  ...
  private async importFiles(): Promise<void> {
    const { host, user, port, authMethod, password, keyPath, remoteFiles } = this.config.remote;
  ...
  async pull(options: PullOptions = {}) {
    ...
    try {
      ...
      const duration = (Date.now() - startTime) / Executer.TIME_UNITS.MS_PER_SECOND;
